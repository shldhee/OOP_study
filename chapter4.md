...
* [chapter4 정리](#chapter4-정리)
	* [분석](#분석)
		* [1. 본문 분석](#1-본문-분석)
			* [클래스들과 메소들을 찾아내기 위해 유스케이스에서 명사(와 동사)를 분석하는 것을 **본문 분석**이라고 합니다.](#클래스들과-메소들을-찾아내기-위해-유스케이스에서-명사와-동사를-분석하는-것을-본문-분석이라고-합니다)
		* [2. 명사는 객체입니다.](#2-명사는-객체입니다)
			* [여기에는 `Bark` 클래스가 없습니다.](#여기에는-bark-클래스가-없습니다)
		* [3. 유스케이스에서 단어들은 중요합니다.](#3-유스케이스에서-단어들은-중요합니다)
		* [4. 바보같은 질문이란 없습니다.(p.209)](#4-바보같은-질문이란-없습니다p209)
			* [잘 만든 유스케이스는 시스템이 하는 일을 **명확히** 그리고 **정확히** 이해하기 쉬운 언어로 설명합니다. 유스케이스를 잘 만든 후에, 유스케이스의 **본문 분석**을 통해 시스템에서 필요한 클래스들을 빠르고 쉽게 찾을 수 있습니다.](#잘-만든-유스케이스는-시스템이-하는-일을-명확히-그리고-정확히-이해하기-쉬운-언어로-설명합니다-유스케이스를-잘-만든-후에-유스케이스의-본문-분석을-통해-시스템에서-필요한-클래스들을-빠르고-쉽게-찾을-수-있습니다)
		* [5. 분석의 힘](#5-분석의-힘)
			* [왜 `Dog` 클래스가 없을까요?](#왜-dog-클래스가-없을까요)
			* [유스케이스 안의 **명사들**이 클래스가 되지 않더라도 집중하세요.](#유스케이스-안의-명사들이-클래스가-되지-않더라도-집중하세요)
			* [클래스들이 어떻게 유스케이스가 설명하는 시스템의 동작을 도와줄 수 있는 지 생각해보세요.](#클래스들이-어떻게-유스케이스가-설명하는-시스템의-동작을-도와줄-수-있는-지-생각해보세요)
		* [6. 동사는 오퍼레이션입니다.](#6-동사는-오퍼레이션입니다)
			* [코드 자석](#코드-자석)
			* [연필을 깍으며 해답 : 왜 `Dog` 클래스가 없을까요?](#연필을-깍으며-해답-왜-dog-클래스가-없을까요)
		* [7. 바보같은 질문이란 없습니다.(p215)](#7-바보같은-질문이란-없습니다p215)
		* [8. 분석을 잘해서 적당한 클래스 찾기, 연관과 다중도](#8-분석을-잘해서-적당한-클래스-찾기-연관과-다중도)
			* [마리아의 강아지문 클래스 다이어그램](#마리아의-강아지문-클래스-다이어그램)
			* [연필을 깍으며 : `DogDoor` 클래스의 `allowedBarks` 속성으로 어떤 타입들을 사용할 수 있을까요?](#연필을-깍으며-dogdoor-클래스의-allowedbarks-속성으로-어떤-타입들을-사용할-수-있을까요)
		* [9. 왜 클래스 다이어그램을 사용하죠?](#9-왜-클래스-다이어그램을-사용하죠)
			* [클래스 다이어그램이 전부는 아닙니다.](#클래스-다이어그램이-전부는-아닙니다)
		* [10. 다이어그램에서 뭐가 빠진거죠?](#10-다이어그램에서-뭐가-빠진거죠)
			* [`recognize()`는 어떻게 동작하나요?](#recognize는-어떻게-동작하나요)
		* [11. 핵심정리](#11-핵심정리)
		* [12. 코딩 조각들 채우기 디자인 퍼즐](#12-코딩-조각들-채우기-디자인-퍼즐)
		* [13. 나의 정의는 무엇일까요?](#13-나의-정의는-무엇일까요)
